...
"""
Job-Shop-Scheduling mit Hexaly.
...
def _read_all_ints(path: str) -> List[int]:
    """Liest alle Integer-Werte aus einer Datei."""
    ints: List[int] = []
    ...
    with open(path, "r", encoding="utf-8") as f:
        for raw in f:
            ...
            # Kommas durch Leerzeichen ersetzen und alle Tokens prüfen.
            for tok in s.replace(",", " ").split():
                try:
                    ints.append(int(tok))
                except ValueError:
                    # Nicht interpretierbare Tokens werden ignoriert.
                    pass

    return ints


def read_abz_ft_la_pairs(
    path: str,
) -> Tuple[int, int, List[List[int]], List[List[int]], int]:
    """
    ...
    if len(data) < need:
        raise ValueError(
            f"zu wenig Integers. Erwartet >= {need}, gefunden {len(data)}."
        )
    ...
    # Lese für jeden Job die Folge aus (machine_id, processing_time)-Paaren.
    ...
    # Falls Maschinen 1-basiert kodiert sind, auf 0-basierte Indizes umstellen.
    ...
    processing_time: List[List[int]] = [
        [0] * nb_machines for _ in range(nb_jobs)
    ]

    # Verarbeitung in die interne Repräsentation überführen.
    for j, ops in enumerate(jobs_ops):
        ...
        for m, p in ops:
            if not (0 <= m < nb_machines):
                raise ValueError(
                    f"machine id {m} außerhalb 0..{nb_machines - 1}"
                )
...
def read_taillard_instance(
    path: str,
) -> Tuple[int, int, List[List[int]], List[List[int]], int]:
    """
    Liest eine Instanz im Taillard/JSP-lib-Format ein.

    Entspricht funktional deinem bisherigen Parser.
    """
    with open(path, "r", encoding="utf-8") as f:
    ...
    # Bearbeitungszeiten in Verarbeitungsreihenfolge einlesen.
    ...
    # Maschinenreihenfolge einlesen und von 1-basiert auf 0-basiert umstellen.
    machine_order = [
        [int(lines[i].split()[j]) - 1 for j in range(nb_machines)]
        for i in range(mo_start, mo_end)
    ]

    # Auf interne Matrix processing_time[j][m] umformen.
    processing_time = [
        [
            processing_times_in_processing_order[j][machine_order[j].index(m)]
...
def read_instance_auto(
    path: str,
) -> Tuple[int, int, List[List[int]], List[List[int]], int]:
    """Versucht zuerst ABZ/FT/LA-Format, sonst Taillard-Format."""
    try:
...
def generate_setup_times(
    ...
    setup_time[m][j_prev][j_next] liegt in [0, max_setup]
    ...
    setup_time = [
        [
            [random.randint(0, max_setup) for _jn in range(nb_jobs)]
            for _jp in range(nb_jobs)
        ]
        for _m in range(nb_machines)
    ]

    # Optional: Keine Setupzeit beim Übergang eines Jobs auf sich selbst.
    if diagonal_zero:
        for m in range(nb_machines):
            for j in range(nb_jobs):
                setup_time[m][j][j] = 0
...
def solve_hexaly_code2_model(
    ...
    # Setupzeiten: obere Schranke max_end bei Bedarf vergrößern.
    ...
    with hexaly.optimizer.HexalyOptimizer() as optimizer:
        ...
        # Für jedes Job-Maschinen-Paar ein Intervall mit fester Länge.
        tasks = [
            [model.interval(0, max_end) for m in range(nb_machines)]
            for j in range(nb_jobs)
        ]
        ...
        for j in range(nb_jobs):
            for m in range(nb_machines):
                model.constraint(
                    model.length(tasks[j][m]) == int(processing_time[j][m])
                )

        task_array = model.array(tasks)

        # Technologische Reihenfolge der Operationen pro Job.
        for j in range(nb_jobs):
            for k in range(nb_machines - 1):
                m1 = machine_order[j][k]
                m2 = machine_order[j][k + 1]
                model.constraint(
                    model.start(model.at(task_array, j, m2))
                    >= model.end(model.at(task_array, j, m1))
                )
        ...
        # Reihenfolge auf jeder Maschine mit optionalen Setupzeiten.
        for m in range(nb_machines):
            seq = jobs_order[m]
            ...
            if setup_arr is None:
                seq_lambda = model.lambda_function(
                    lambda i: model.start(
                        model.at(task_array, seq[i + 1], m)
        ...
        # Gesamtzeit / Makespan minimieren.
        ...
        schedule: Dict[int, List[Tuple[float, float, int, int]]] = {
            m: [] for m in range(nb_machines)
        }

        # Lösung in ein maschinenweises Schedule-Dictionary überführen.
        for j in range(nb_jobs):
            for m in range(nb_machines):
                iv = tasks[j][m].value
                schedule[m].append((float(iv.start()), float(iv.end()), j, m))

        # Je Maschine nach Startzeit sortieren.
        for m in schedule:
...
def plot_gantt(
    ...
    """Erstellt ein Gantt-Diagramm für das berechnete Schedule."""
    ...
    max_t = (
        max(et for mach in schedule for (_, et, _, _) in schedule[mach])
        if schedule
        else 0
    )
...
def solve_from_jsplib(
    ...
    """Lädt eine Instanz, löst sie und optional visualisiert das Ergebnis."""
    (
        nb_jobs,
        nb_machines,
        processing_time,
    ...
    if visualize:
        plot_gantt(
            schedule,
            title=f"JSSP Hexaly (Makespan={cmax:.2f})",
            save_path="gantt.png",
        )
