...
"""
Job-Shop-Scheduling mit Google OR-Tools.

Dieses Modul enthält Funktionen zum:
- Erzeugen zufälliger Setupzeiten
- Modellieren des Job-Shop-Scheduling-Problems mit CP-SAT
- Lösen der Instanz mit Google OR-Tools
- Rückgeben der berechneten Start- und Endzeiten aller Operationen
...
class TaskVars:
    """Container für die Variablen einer einzelnen Operation."""
...
def _build_random_setup_times(
    ...
    """
    Erzeugt zufällige Setup-Zeiten.

    setup_times[machine][job_from][job_to] liegt in [0, max_setup]
    und setup_times[m][j][j] ist immer 0.
    """
...
def solve_jobshop(
    ...
    *,
    ...
    """
    Löst ein Job-Shop-Scheduling-Problem.

    jobs_data: list[job], wobei job = list[(machine, duration)]

    Falls use_setup_times=True und setup_times=None:
    -> es werden zufällige Setup-Zeiten in
       [0, max_duration_in_instance] erzeugt.
    """

    # Grundlegende Instanzparameter bestimmen.
    jobs_count = len(jobs_data)
    machines_count = 1 + max(task[0] for job in jobs_data for task in job)
    horizon = sum(task[1] for job in jobs_data for task in job)
    max_duration_in_instance = max(
        task[1] for job in jobs_data for task in job
    )

    # Falls gewünscht, aber keine Setup-Matrix übergeben wurde:
    # zufällige Setup-Zeiten generieren.
    ...
    # Variablenerstellung für jede Operation jedes Jobs.
    ...
    # Entweder ohne Setup-Zeiten per NoOverlap oder mit expliziten
    # Reihenfolgebedingungen inklusive Setup-Zeiten.
            ...
            for i in range(len(tasks)):
                for j in range(i + 1, len(tasks)):
                    ...
                    a_before_b = model.new_bool_var(
                        f"m{machine}_a{a}_before_b{b}"
                    )

                    model.add(
                        all_tasks[a].end
                        + setup_times[machine][a_job][b_job]
                        <= all_tasks[b].start
                    ).only_enforce_if(a_before_b)

                    model.add(
                        all_tasks[b].end
                        + setup_times[machine][b_job][a_job]
                        <= all_tasks[a].start
                    ).only_enforce_if(a_before_b.Not())

    # Technologische Reihenfolge innerhalb jedes Jobs:
    # Aufgabe t+1 darf erst starten, wenn Aufgabe t beendet ist.
    for job_id, job in enumerate(jobs_data):
        for t in range(len(job) - 1):
            model.add(
                all_tasks[(job_id, t + 1)].start
                >= all_tasks[(job_id, t)].end
            )

    # Gesamtzeit (Makespan) definieren und minimieren.
